            ...
            <section class="featured-section">
                <div class="container-fluid">
                    <div class="featured-slider">
                        <?php
                        $magazine_prime_ticker_news_args = array(
                            'post_type' => 'post',
                            'ignore_sticky_posts' => true,
                            'posts_per_page' => 6,
                        ); ?>
                        <?php $magazine_prime_ticker_news_post_query = new WP_Query($magazine_prime_ticker_news_args);
                        if ($magazine_prime_ticker_news_post_query->have_posts()) :
                            while ($magazine_prime_ticker_news_post_query->have_posts()) : $magazine_prime_ticker_news_post_query->the_post();
                                if (has_post_thumbnail()) {
                                    $thumb = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'thumbnail');
                                    $url = $thumb['0'];
                                } else {
